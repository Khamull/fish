...
<jsp:useBean id="func" class="cadastro.Funcionario" scope="page"></jsp:useBean>
...
//Recupera lista de todos os produtos
rs01 = st01.executeQuery(produto.listaProdutos());
//Recupera Lista de Usuários
rs02 = st02.executeQuery(func.listaFuncionarioAtivos());
...
function calculaPerda(obj)
{
	var qtdFim = obj.value;
	var qtdInic = document.form1.quantidade.value;
	document.form1.perda.value = qtdInic -qtdFim; 
}
...
<tr>
 ...
 <table width="366" align="center" cellpadding="4" cellspacing="4" style="border:1px solid #666666">
  ...
  <tr>
   <td width="168" align="left">Destino:</td>
   <td width="170" align="left"><select id="produtoDestino" name="destino"><option selected="selected" value="">Selecione destino</option> 
   <%while(rs01.next())
   {%>
   															<option value="<%=rs01.getString("produtoID")%>"><%=rs01.getString("nome") %></option>
   <%} %>
   <%=unidadeMedida %></td>
  </tr>
   <tr>
   <td width="168" align="left">Quantidade Final</td>
   <td width="170" align="left"><input type="text" name="qtdFinal" onKeyPress="return numero(this)" onblur="" onchange="calculaPerda(this)" size="20" maxlength="10" /> 
   <%=unidadeMedida %></td>
  </tr>
  <tr>
   <td width="168" align="left">Perda</td>
   <td width="170" align="left"><input type="text" name="perda" onKeyPress="return numero(this)" onblur="" size="20" maxlength="10" readonly="readonly"/> 
   <%=unidadeMedida %></td>
  </tr>
  <td width="168" align="left">Responsavel</td>
   <td width="170" align="left"><select id="responsavel" name="responsavel"><option selected="selected" value="">Selecione responsavel</option> 
   <%while(rs02.next())
   {%>
   															<option value="<%=rs02.getString("funcionarioNome")%>"><%=rs02.getString("funcionarioNome") %></option>
   <%} %>
   <%=unidadeMedida %></td>
  </tr>
  <!-- tr>
  ...
  <tr-->
   ...
   <td align="center" colspan="2"><input type="hidden" name="produtoID" value="<%=produto.produtoID %>">
   <input type="hidden" name="quantidadeEstoque" value="<%=quantidadeEstoque %>"><input name="CADASTRAR" type="submit" class="botao" value="Retirar" /></td>
  </tr>
 </table>
 </form>
 </td>
